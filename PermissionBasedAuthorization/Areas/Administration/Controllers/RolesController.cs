...
namespace Rusella.App.Areas.Administration.Controllers
{
    [Area("Administration")]
    ...
    public class RolesController : BaseController
    {
        ...
        public async Task<IActionResult> ManagePermissions(string roleId)
        {
            ...
            var allClaims = Permissions.GenerateAllPermissionsGroupedByModule(); // Method to return grouped permissions

            var allPermissions = new Dictionary<string, List<CheckBoxViewModel>>();

            foreach (var module in allClaims)
            {
                allPermissions[module.Key] = module.Value.Select(p => new CheckBoxViewModel
                {
                    DisplayValue = p,
        ...
        public async Task<IActionResult> ManagePermissions(PermissionsFormViewModel model)
        {
            ...
            var selectedClaims = model.RoleClaims.SelectMany(m => m.Value)
